...
const emit = defineEmits(['checkInSuccess', 'checkOutSuccess']);

const attendanceMode = ref('WFO');
...
const distanceFromOffice = ref(null);
...
const isCameraActive = ref(getSavedCameraState());
...
const toggleCamera = () => {
    if (isCameraActive.value) {
        stopCamera();
        localStorage.setItem(CAMERA_STATE_KEY, 'false');
    } else {
        startCamera();
        localStorage.setItem(CAMERA_STATE_KEY, 'true');
    }
};
...
const hasCheckedIn = computed(() => presenceData.value?.check_in_time !== null && presenceData.value?.check_in_time !== undefined);
const hasCheckedOut = computed(() => presenceData.value?.check_out_time !== null && presenceData.value?.check_out_time !== undefined);

const isButtonEnabled = computed(() => {
    if (isSubmitting.value) return false;
    if (hasCheckedOut.value) return false; // Already done for today
...
const buttonText = computed(() => {
    ...
    if (hasCheckedOut.value) return `Sudah Check-Out (${presenceData.value.check_out_time})`;
    if (hasCheckedIn.value) return `Keluar Sekarang - ${currentTime.value}`;
...
const actionButtonClass = computed(() => {
    ...
    if (hasCheckedIn.value && isButtonEnabled.value) {
...
const handleSubmit = async () => {
    ...
    try {
        const isCheckOut = hasCheckedIn.value;
        const endpoint = isCheckOut ? '/intern/presence/check-out' : '/intern/presence/check-in';

        const payload = {
            photo: capturedImage.value,
            latitude: userLocation.value?.[0],
            longitude: userLocation.value?.[1],
            distance_meters: distanceFromOffice.value,
        };
        ...
        if (response.data.success) {
            ...
            presenceData.value = response.data.presence;
            capturedImage.value = null;
        ...
        submitError.value = error.response?.data?.message || 'Terjadi kesalahan. Silakan coba lagi.';
...
<template>
    <section class="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            <div class="flex items-center gap-4">
                ...
                <div class="flex bg-slate-100 p-1 rounded-lg">
                    <button @click="attendanceMode = 'WFO'"
                        class="px-3 py-1 rounded-md cursor-pointer text-xs font-bold transition-all flex items-center gap-1.5"
                    ...
                    <button @click="attendanceMode = 'WFH'"
                        class="px-3 py-1 rounded-md cursor-pointer text-xs font-bold transition-all flex items-center gap-1.5"
                        :class="attendanceMode === 'WFH' ? 'bg-white text-primary shadow-sm' : 'text-slate-500 hover:text-slate-700'">
                        <Home class="w-3.5 h-3.5" /> WFH/WFA
                    </button>
                </div>
            </div>

            <div class="flex items-center gap-2">
                <span v-if="!hasCheckedIn && countdown"
                    class="text-xs font-mono px-3 py-1 rounded-full border flex items-center gap-1"
                    :class="countdown === 'Terlambat' ? 'bg-red-50 text-red-600 border-red-200' : 'bg-amber-50 text-amber-600 border-amber-200'">
                    <span class="material-symbols-outlined scale-75">schedule</span>
                    {{ countdown === 'Terlambat' ? 'Sudah lewat jam 8' : countdown }}
                </span>
                <span v-if="distanceFromOffice !== null" class="text-xs font-mono font-bold"
                    :class="isInRange ? 'text-success' : 'text-danger'">
                    {{ formattedDistance }}
                </span>
                <span
                    class="text-xs font-mono bg-slate-100 px-3 py-1 rounded-full text-text-secondary border border-slate-200 flex items-center gap-1">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div
                class="relative group overflow-hidden rounded-xl h-64 lg:h-80 w-full bg-slate-100 border border-slate-200 shadow-inner">
                <div ref="mapContainer" class="absolute inset-0 z-0"></div>

                <div class="absolute inset-x-4 bottom-4 z-10">
                    <div
                        class="bg-white/95 backdrop-blur-sm p-3 rounded-lg flex items-center gap-3 shadow-lg border border-slate-100">
                        <div class="p-2 rounded-md"
                            :class="isInRange ? 'bg-blue-50 text-success' : 'bg-red-50 text-danger'">
                            <MapPin class="w-5 h-5" />
                        </div>
                        <div class="flex-1 min-w-0">
                            <p class="text-[10px] font-bold uppercase text-text-secondary tracking-wider">Zona Saat Ini
                            </p>
                ...
                <div v-if="isLoadingLocation"
                    class="absolute inset-0 bg-white/50 backdrop-blur-sm flex items-center justify-center z-20">
            ...
            <div class="flex flex-col gap-4 h-full">
                <div
                    class="relative flex-1 bg-slate-900 rounded-xl overflow-hidden min-h-[220px] flex items-center justify-center group border border-slate-800 shadow-inner">
                    <video ref="videoElement" autoplay playsinline muted
                        class="absolute inset-0 w-full h-full object-cover transform scale-x-[-1]"
                        :class="{ 'opacity-0': capturedImage || !isCameraActive }"></video>

                    <img v-if="capturedImage" :src="capturedImage"
                        class="absolute inset-0 w-full h-full object-cover transform scale-x-[-1]" />

                    <div v-if="!capturedImage && isCameraActive"
                        class="absolute inset-0 bg-gradient-to-b from-transparent via-emerald-500/10 to-transparent z-10 animate-scan">
                    </div>

                    <div v-if="!isCameraActive && !capturedImage && !cameraError"
                        class="absolute inset-0 bg-slate-900 flex flex-col items-center justify-center text-slate-500 z-10">
                        <VideoOff class="w-12 h-12 mb-2 opacity-50" />
                        <span class="text-sm font-medium">Kamera Non-aktif</span>
                    </div>

                    <div v-if="cameraError"
                        class="absolute inset-0 bg-slate-800 flex flex-col items-center justify-center text-slate-400 p-4 text-center z-20">
                    ...
                    <div class="absolute top-4 right-4 z-20 flex gap-2">
                        <button @click="toggleCamera"
                            class="p-2 rounded-full bg-black/40 text-white backdrop-blur-md hover:bg-black/60 transition-colors"
                            title="Toggle Camera">
                            <component :is="isCameraActive ? Video : VideoOff" class="w-4 h-4" />
                        </button>

                        <div v-if="isCameraActive && !capturedImage"
                            class="bg-danger/90 backdrop-blur text-white text-[10px] font-bold px-2 py-1 rounded animate-pulse flex items-center gap-1 shadow-sm">
                    ...
                    <div class="absolute bottom-4 inset-x-0 flex justify-center z-20">
                        <button v-if="!capturedImage" @click="capturePhoto" :disabled="!isCameraActive"
                            class="w-14 h-14 rounded-full cursor-pointer border-4 border-white bg-white/20 hover:bg-white/40 transition-all flex items-center justify-center backdrop-blur-sm active:scale-95 shadow-lg disabled:opacity-50 disabled:cursor-not-allowed">
                            <div class="w-10 h-10 rounded-full bg-white"></div>
                        </button>
                        <button v-else @click="retakePhoto"
                            class="px-4 py-2 rounded-full cursor-pointer bg-slate-900/80 text-white backdrop-blur-md border border-white/20 flex items-center gap-2 hover:bg-black transition-all">
                ...
                <div v-if="submitSuccess"
                    ...
                    <span class="text-sm font-medium">{{ submitSuccess }}</span>
                </div>
                ...
                <div v-if="hasCheckedIn && !hasCheckedOut && !submitSuccess"
                    class="flex items-center gap-3 p-4 bg-blue-50 border border-blue-200 rounded-xl text-blue-700">
                    <CheckCircle class="w-5 h-5 shrink-0" />
                ...
                <button @click="handleSubmit" :disabled="!isButtonEnabled"
                    class="w-full py-4 text-white cursor-pointer rounded-xl font-bold text-lg shadow-lg transition-all active:scale-95 flex items-center justify-center gap-3 group/btn relative overflow-hidden"
                    :class="actionButtonClass">
                    <div
                        class="absolute inset-0 bg-white/20 translate-y-full group-hover/btn:translate-y-0 transition-transform duration-300">
                    </div>
                    <span
                        class="bg-white/20 rounded-full p-1.5 relative z-10 transition-transform group-hover/btn:scale-110">
                        <LogOut v-if="hasCheckedIn && !hasCheckedOut" class="w-6 h-6" />
                        <Fingerprint v-else class="w-6 h-6" />
                    </span>
