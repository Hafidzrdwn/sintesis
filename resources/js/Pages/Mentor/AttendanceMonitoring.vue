...
import {
  ...
  CheckCircle2, Building2, Home, X, MapPin, Image, FileText, Eye
...
const applyFilters = () => {
  router.get(route('mentor.attendance'), {
...
watch([selectedMonth, selectedYear, selectedStatus], () => applyFilters());
  ...
  <div class="flex flex-col gap-6">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div>
        ...
        <p class="text-slate-500 mt-1">Pantau kehadiran intern yang Anda bimbing.</p>
      </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
      <div class="bg-white p-4 rounded-xl border border-slate-200 text-center">
        ...
        <p class="text-2xl font-black text-slate-900">{{ overallStats?.total_mentees || 0 }}</p>
    ...
    <div v-if="menteeStats?.length" class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      <div class="p-4 border-b border-slate-200 bg-slate-50/50">
        <h3 class="font-bold text-slate-900">Statistik per Mentee</h3>
      </div>
      <div class="overflow-x-auto">
        <table class="w-full text-left text-sm">
          ...
          <tbody class="divide-y divide-slate-100">
            <tr v-for="mentee in menteeStats" :key="mentee.id" class="hover:bg-slate-50">
              <td class="px-6 py-4">
                <p class="font-medium text-slate-900">{{ mentee.name }}</p>
                <p class="text-xs text-slate-400">{{ mentee.email }}</p>
              </td>
              <td class="px-6 py-4 text-center">
                <span class="px-2 py-1 rounded-full bg-emerald-50 text-emerald-600 text-xs font-bold">{{ mentee.present
                  }}</span>
              </td>
              <td class="px-6 py-4 text-center">
                <span class="px-2 py-1 rounded-full bg-amber-50 text-amber-600 text-xs font-bold">{{ mentee.late
                  }}</span>
              </td>
              <td class="px-6 py-4 text-center">
                <span class="px-2 py-1 rounded-full bg-blue-50 text-blue-600 text-xs font-bold">{{ mentee.leave
                  }}</span>
              </td>
              <td class="px-6 py-4 text-center">
                <span class="px-2 py-1 rounded-full text-xs font-bold"
                  :class="mentee.absent > 0 ? 'bg-red-50 text-red-600' : 'bg-slate-50 text-slate-400'">{{ mentee.absent
                  }}</span>
              </td>
              <td class="px-6 py-4 text-center font-mono text-slate-600">{{ mentee.total_hours }} jam</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left text-sm">
          ...
          <tbody class="divide-y divide-slate-100">
            <tr v-for="att in attendances" :key="att.id" class="hover:bg-slate-50">
